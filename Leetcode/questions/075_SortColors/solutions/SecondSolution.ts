...
function swap <T>(list: T[], idx1: number, idx2: number): void {
    let tmp: T = list[idx1]
    list[idx1] = list[idx2]
    list[idx2] = tmp
}

export default function SecondSolution (nums: number[]): number[] {
    // [0, ..., zero] 的值均为 0
    let zero = -1,
